...
## lb Project Scope (REQUIRED)

**`lb` write commands are blocked unless `LB_TEMP_NAME` is set.** This is enforced at the shell level.

At the start of every session, set the project:

```bash
export LB_TEMP_NAME='AWS Infrastructure'
# or another active DevOps project:
# export LB_TEMP_NAME='Platform 2.0 Infrastructure'
# export LB_TEMP_NAME='ECS Env Var DevEx — SSM Parameter Store Migration'
# export LB_TEMP_NAME='DevOps Ad-hoc Requests'
```

`LB_TEMP_NAME_MODE=project` is set automatically — you only need `LB_TEMP_NAME`. If the project is unclear, ask the user before any `lb create` calls.
